...
Derives per-metric detection thresholds EMPIRICALLY from quiet-window data
(a period where no known drift event occurred), rather than picking
numbers by convention (e.g. a blanket "3-sigma" or "alpha=0.05" applied
uniformly). This is the step locked in during planning: run the eval
suite for 1-2+ weeks against a stable model, learn what "normal wobble"
looks like per metric, THEN derive thresholds from that -- and lock them
before ever looking at real transition/validation data, to avoid
circularity (calibrating and validating against the same events would
make any reported recall/FPR meaningless).

Why robust statistics (median/MAD) instead of mean/std as the default:
  - accuracy and format_adherence are proportions from a SMALL n per run
    (20 and 6 calls respectively) -- lumpy, not smoothly Gaussian.
  - refusal_rate on the safe XSTest subset (5 prompts) is even lumpier --
    it can only take values like 0/5, 1/5, 2/5, etc.
  - latency is typically right-skewed (occasional slow calls drag the
    mean up more than the median).
  Mean/std is still computed and reported alongside, for reference and
  because some downstream detectors (PELT's penalty term) conventionally
  use a variance estimate -- but median/MAD is what drives the actual
  threshold by default, since it's less distorted by the outliers that
  are common in this kind of small-n operational data.

Each metric has a DIRECTION: which side of "changed" actually indicates
a problem. Accuracy/format dropping is bad; refusal rate (safe subset)
rising is bad; latency rising is bad. This matters because a symmetric
threshold would be wrong for all of these -- e.g. accuracy going UP
isn't something we need to alarm on.
"""
