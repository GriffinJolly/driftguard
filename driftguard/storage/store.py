"""
Storage layer for DriftGuard.

SQLite by default (fine at capstone/validation scale, zero setup, works
identically on Windows/PowerShell). Swappable to Postgres later by just
changing the connection string -- SQLModel/SQLAlchemy handles both.

Three tables:
  - LogRow          : raw DriftLogEntry rows (every LLM call, either path)
  - MetricSeriesRow  : aggregated MetricPoint rows (what detectors consume)
  - ChangePointEventRow : detector outputs (fired alerts, p-values, labels)

This module intentionally has no dependency on the detection/classification
modules -- it is a pure persistence layer they write to and read from.
"""
...
class DriftStore:
    ...
    def update_changepoint_event(self, event: ChangePointEventRow) -> ChangePointEventRow:
        """
        Update an existing ChangePointEventRow (identified by event.id) in
        place. Used by multiple_testing.py to patch in the BH-corrected
        p-value and final significance verdict after bootstrap.py's
        initial, provisional (uncorrected) write -- avoids needing a
        second, separate table for "pre-correction" vs "post-correction"
        state.
        """
        if event.id is None:
            raise ValueError("update_changepoint_event requires event.id to be set (row must already exist)")
        with Session(self.engine) as session:
            session.add(event)
            session.merge(event)
            session.commit()
            refreshed = session.get(ChangePointEventRow, event.id)
            return refreshed
